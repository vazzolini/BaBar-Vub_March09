...
@pattern=("file1","file2","tree1","tree2","outdir","cutFile");
...
sub lastcomp(){
	...
	print OUTFILE "#!/bin/tcsh\n setenv LD_LIBRARY_PATH \${LD_LIBRARY_PATH}:$ENV{DIRLIB}\n";
       	print OUTFILE "$ENV{BFDIST}/releases/$ENV{BFCURRENT}/bin/$ENV{BFARCH}/bbrroot -b -q '$ENV{CWD}/comp.C(\"$ARG{file1}\",\"$ARG{tree1}\",\"$ARG{file2}\",\"$ARG{tree2}\",\"$ARG{outdir}\",\"$ARG{cutFile}\",\"$histos[1]\",$histos[2],$histos[3],$histos[4],$histos[5],$histos[6])'\n";
	close OUTFILE;
    }
