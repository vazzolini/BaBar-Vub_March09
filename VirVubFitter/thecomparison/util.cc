...
void setHist(TH1 *h, Int_t color, Int_t symbol, Double_t size, Double_t width) {
  h->SetLineColor(color);   h->SetLineWidth((Width_t)width);
...
// ----------------------------------------------------------------------
void mk4Vector(TLorentzVector &p4, const float p, const float t, const float f, const float m) {
...
double chi2Test(TH1 *h1, TH1 *h2, double& chi2, double& ndof, int constrain) {
  ...
  double chsq(0.), a1(0.), a2(0.), A1(h1->GetSumOfWeights()), A2(h2->GetSumOfWeights());
  for (int i = 1; i <= nbins; ++i) {
    a1 = h1->GetBinContent(i);
    a2 = h2->GetBinContent(i);
    if ((TMath::Abs(a1) < 1.e-8) && (TMath::Abs(a2) < 1.e-8)) {
      cout << "chi2Test: Skipping bin with < 1.e-8 entries" << endl;
      df -= 1.;
    } else if ((a1 < 0.) || (a2 < 0.)) {
      cout << "chi2Test: Skipping bin with negative entries" << endl;
      df -= 1.;
    } else if ((a1/A1 < 0.01) || (a2/A2 < 0.01)) {
      cout << "chi2Test: Skipping bin with less than 1\% contribution" << endl;
      df -= 1.;
    } else {
