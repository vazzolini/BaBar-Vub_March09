...
//List of branches
   ...
   makedep(TTree *tree=0);
   ~makedep();
   Int_t  Cut(Int_t entry);
   Int_t  GetEntry(Int_t entry);
   Int_t  LoadTree(Int_t entry);
   void   Init(TTree *tree);
   void   Loop(char *pippo, bool mctruth);
   void   bookHist();
   Bool_t Notify();
...
Int_t makedep::GetEntry(Int_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Int_t makedep::LoadTree(Int_t entry)
{
