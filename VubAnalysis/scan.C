...
void scan(TString thefile, char *thename, double ref, char * type = ""){
   ...
   TH1D brbr("brbr","brbr",30,0.008,0.0295);
   pulls.SetStats(0);
   brbr.SetStats(0);
   ...
   double x = 0.15;
   char myline[100];
   sprintf(myline, "mean = %6.4f +/- %5.4f", mean, errmean); 
   tl.DrawTextNDC(x, 0.8, myline);
   sprintf(myline, "sigma = %6.4f +/- %5.4f", sigma, errsigma); 
